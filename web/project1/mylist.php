...
<?php
   ...
   $db = get_db();
   $movie = $db->prepare("SELECT mt.title FROM movie mt INNER JOIN list l ON mt.id = l.movie_id INNER JOIN user_profile p ON l.user_id = p.id WHERE l.user_id = 1")// p.id");
   $movie->execute();
